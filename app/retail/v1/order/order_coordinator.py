...
class OrderCoordinator(BaseCoordinator):
    ...
    def get_status(self, order_id_list, identifier_instance_id):
        query ='''SELECT posr.order_id, posr.order_status, posr.fulfilment_status, posr.refund_status 
        FROM plotch_order_status_request AS posr JOIN (SELECT order_id, MAX(created_at) AS max_created_at  
        FROM plotch_order_status_request WHERE order_id IN ({}) AND storefront_id = '{}' GROUP BY order_id) 
        AS max_entry ON posr.order_id = max_entry.order_id AND posr.created_at = max_entry.max_created_at
        '''.format(str(order_id_list)[1:-1], identifier_instance_id)
        return self.mysql_conn_pool.query_db_pool(query)
