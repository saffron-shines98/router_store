...
class CatalogService:
    ...
    def fetch_catalog_count(self):
        authenticate_user_from_through_sso = authenticate_user(self.headers.get('Auth-Token'), self.headers.get('Nodesso-Id'))
        count_object = [{'col':'instance_id', 'val': self.params.get('noderetail_storefront_id')}, {'col':'instance_type_id', 'val': 46}]
        try:
            plotch_instance = self.coordinator.get_single_data_from_db('plotch_instance',count_object)
        except:
            plotch_instance = self.coordinator.get_single_data_from_db('plotch_instance',count_object)
        instance_details = plotch_instance.get('instance_details')
        try: 
